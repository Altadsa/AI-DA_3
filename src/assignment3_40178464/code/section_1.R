...
#Please remember to set the working directory in Session->Set Working Directory -> Source File Location

#Load Feature Data
filepath <- paste(getwd(),"/40178464_features.csv", sep = "")
...
for (f_name in c("span", "cols_with_5", "neigh5"))
{
...
#Below is code which I used in conjunction with visually assessing the results from assignment 2. This code should not be run with any of the code above
# as it will change the results I obtained for each task. Instead, I have stored the results for these tests in separate csv files if you want to look at them.

#For 1.3

#I highly advise against running this code, as it took a hyperthreaded 8 core processor over 20 mins to complete.
#I used this to determine which features yielded the highest prediction accuracy over the dataset.
#This is not intended to be run since it will result in different results for the remaining sections compared to the report.
#See the '1.3_combn_table.csv' for results
...
# 
# }
# write.csv(combo_class_table, "1.3_combn_table.csv")

#For 1.5

#Detemrine which features might improve the accuracy of the model
#This code takes some time to run. I used it for the purpose of finding which model had
#the highest accuracy using the 3 features from 1.3 and an additional 4th
#The results can be found in '1.5_combn_table'
...
#   }
# }
# write.csv(log_4_table, "1.5_combn_table.csv")
